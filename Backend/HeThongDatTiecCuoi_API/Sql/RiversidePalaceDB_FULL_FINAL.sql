...
-- 5. DangNhapNgoai (Google và các nhà cung cấp đăng nhập ngoài)
CREATE TABLE DangNhapNgoai (
    ...
    NgayLienKet DATETIME2(0) NOT NULL CONSTRAINT DF_DangNhapNgoai_NgayLienKet DEFAULT SYSUTCDATETIME(),
    CONSTRAINT PK_DangNhapNgoai PRIMARY KEY (DangNhapNgoaiID),
    CONSTRAINT UQ_DangNhapNgoai_ProviderUser UNIQUE (NhaCungCap, MaNguoiDungNhaCungCap),
    CONSTRAINT UQ_DangNhapNgoai_UserProvider UNIQUE (NguoiDungID, NhaCungCap),
    CONSTRAINT FK_DangNhapNgoai_NguoiDung FOREIGN KEY (NguoiDungID)
        REFERENCES NguoiDung(NguoiDungID) ON DELETE CASCADE
);
GO
